...
public class TelegramMsgSenderImpl implements TelegramMsgSender {
	...
	@Override
	public void sendKeyboard(TelegramBotApi arg0, String msg, long chatId, ReplyMarkup replyMarkup) {
		try {
	...
	public void sendMessage(TelegramMessage message) {
		...
		case TelegramMessage.SENDER_MODE_TEXT:
			sendTextMessage(message.getApi(), message.getMessage(), message.getChatId());
			break;
		case TelegramMessage.SENDER_MODE_KEYBOARD:
			sendKeyboard(message.getApi(), message.getMessage(), message.getChatId(), message.getReplyMarkup());
			break;
		case TelegramMessage.SENDER_MODE_MULTI:
			sendTextMessage(message.getApi(), message.getMessage(), message.getChatId());
			sendKeyboard(message.getApi(), message.getMessage(), message.getChatId(), message.getReplyMarkup());
			break;
		default:
