...
public class DeviceSecurityServiceImpl implements DeviceSecurityService {
	...
	@Value("${smarthouse.server.security.disabled}")
	...
	public String processDeviceRegistrationRequest(DeviceRequest request) throws DeviceRequestValidateException {
		...
		String unhashedDeviceToken = SecurityUtil.split(serverKeyHash, tempServerKey, tempDeviceKey);

		String unhashedServerToken = SecurityUtil.split(tempServerKey, serverKey, request.getDeviceId());
		...
		secContext.put(request.getDeviceId(), serverHash, deviceHash, tempServerKey);
	...
	public boolean validateDeviceRequestBase(DeviceRequest request) throws DeviceRequestValidateException {
		if (Strings.isEmpty(request.getDeviceId()) && !request.getHeaders().containsKey(HEADER_DEVICE)) {
		...
		/*
		 * if(disabledDeviceIds.containsValue(request.getDeviceId())) { throw new
		 * DeviceRequestValidateException(HttpStatus.FORBIDDEN); }
		 */
	...
	public boolean validateDeviceRequestBaseSecurity(DeviceRequest request) throws DeviceRequestValidateException {
		boolean noAuthHeaders = request.getHeaders() == null
	...
	public boolean validateDeviceRequestRegistration(DeviceRequest request) throws DeviceRequestValidateException {
		...
		if (Strings.isEmpty(request.getDeviceFirmware()) || Strings.isEmpty(request.getDeviceDescr())) {
			throw new DeviceRequestValidateException(HttpStatus.BAD_REQUEST);
		}
