...
bool ESP8266WebServer::_parseForm(WiFiClient& client, String boundary, uint32_t len){
  ...
  if (line == ("--"+boundary)){
    RequestArgument* postArgs = new RequestArgument[MAX_ARGS_COUNT];
    int postArgsLen = 0;
    while(1){
      String argName;
      String argValue;
...
readfile:
    ...
    int iarg;
    int totalArgs = ((MAX_ARGS_COUNT - postArgsLen) < _currentArgCount)?(MAX_ARGS_COUNT - postArgsLen):_currentArgCount;
