...
void JsonDao::init() {
	...
	Serial.println(FPSTR("JsonDao create models"));
	...
	Serial.println(FPSTR("models built"));
	Serial.println(FPSTR("------------------------------------------"));
...
void JsonDao::registerEntityModel(EntityJson* ent) {
	...
	for (const auto& kvp : b.parse(ent->getDescriptor()).as<JsonObject>().get<JsonObject>(JSONKEY_declaredFields)) {
	...
	for (const auto& kvp : b.parse(ent->getDescriptor()).as<JsonObject>().get<JsonObject>(JSONKEY_declaredActions)) {
		JsonArray& currArr =
	...
	JsonObjectUtil::print("Entity MODEL =",json);
	b.clear();
	JsonObjectUtil::print("ROOT =",root);
}
...
void JsonDao::saveEntityModelFile() {
	FileUtils::saveJsonToFile(PATH_MODEL_file, root.get<JsonObject>(ROOT_PATH_MODEL));
}
...
int JsonDao::loadEntityModel(EntityJson* ent,JsonObject& loadedModel) {
	...
	if(i>0){
		Serial.print(FPSTR("fields loaded from file="));
		Serial.println(i);
	}
