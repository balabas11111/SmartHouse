...
class ImpulseRelay: public Entity, public EntityUpdate {
public:
	ImpulseRelay(uint8_t pin, uint8_t signalPin, char* description,
			const char* name, const char* emoji, unsigned long signalTime =
					SIGNAL_TIME, uint8_t onLevel = LOW,
	...
	virtual void doPost(JsonObject& params, JsonObject& response) override {
		...
		if (params.is<char*>(ON_FIELD)
				&& (strcmp("1", params[ON_FIELD]) == 0
						|| strcmp("true", params[ON_FIELD]) == 0)) {
			onPosted = 1;
		}
	...
	virtual void loop() override {
		if (stateChanged) {
			...
			if (on && onTurnedOn != nullptr) {
				onTurnedOn();
			} else if (!on && onTurnedOff != nullptr) {
				onTurnedOff();
			}
