...
public:
	...
	void print() {
		...
		for (uint8_t i = 0; i < itemCount; i++) {
			...
			Serial.print(getDeviceAddress(items[i].uid, i));
	...
	virtual void doAppendFieldsSwg(JsonObject& swgJson) override{
		...
		for (uint8_t i = 0; i < itemCount; i++) {
			...
			EntityDescriptor::appendSwgFieldFloatNameDescrFieldOnly(swgJson, items[i].uidTempKey, items[i].uidDescrKey, EDC_MEASURE_GRAD_CELSIUS);
		}
	}
...
protected:
	...
	String getDeviceAddress(DeviceAddress deviceAddress, uint8_t index) {
		String result = "";
